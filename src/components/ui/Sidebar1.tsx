...
const baseMainItems: SidebarItem[] = [
  ...
  // Ads Manager will be conditionally injected based on packageActive
];
...
const Sidebar: React.FC = () => {
  ...
  const [landscapeShow, setLandscapeShow] = useState<boolean>(false);

  useEffect(() => {
    const mq = window.matchMedia("(orientation: landscape)");
    const check = () => {
      const w = window.innerWidth || document.documentElement.clientWidth;
      setLandscapeShow(mq.matches && w >= 640);
    };
    check();
  ...
  const renderItem = (item: SidebarItem, disabled = false) => {
    ...
    return (
      ...
      >
        ...
        {item.name === "Mine" &&
          (mineOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />)}
      </button>
    );
  };

  // Build mainItems conditionally
  const mainItems = [...baseMainItems];
  if(!authLoading)
  {
    if (user?.packageactive) {
      mainItems.push({ name: "Ads Manager", icon: Megaphone, route: "/ads" });
    } else {
      // Add disabled Ads Manager if you want it visible but disabled
      mainItems.push({ name: "Ads Manager", icon: Megaphone });
    }
  }

  return (
    <aside className={`
        ${landscapeShow ? "flex" : "hidden"}
      ...
      <div className="mb-6">
        ...
        <ul className="space-y-1">
          {mainItems.map((item) => (
            <li key={item.name}>
              {renderItem(item, item.name === "Ads Manager" && !user?.packageactive)}
      ...
      {/* Explore Section */}
      <div className="mb-6">
        <h3 className="text-xs font-semibold text-gray-400 uppercase mb-2">Explore</h3>
        <ul className="space-y-1">
