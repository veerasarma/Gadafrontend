...
export default function AdminGroupCategories() {
  ...
  const load = async () => {
    ...
    const r = await fetch(`${API_BASE_URL}/api/admin/group-categories?` + params, { headers });
  ...
  const remove = async (id: number, name: string) => {
    ...
    const r = await fetch(`${API_BASE_URL}/api/admin/group-categories/${id}`, { method: 'DELETE', headers });
    if (!r.ok) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold">Groups › Categories</h2>
        <Button onClick={() => navigate('/admin/group-categories/new')}>+ Add New Category</Button>
      </div>
            ...
            {rows.map(r => (
              <TableRow key={r.id}>
                ...
                <TableCell className="align-top space-x-2">
                  <Button size="sm" variant="outline" onClick={()=>navigate(`/admin/group-categories/${r.id}`)}>Edit</Button>
