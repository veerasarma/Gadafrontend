...
type PostHit = {
  id: number | string;
  snippet: string;
  author: { id: number | string; username: string; avatar?: string | null };
};
...
export function Navbar() {
  ...
  const flatList = (() => {
    ...
    const top = results.top.map((r) => ({
      group: "Top",
      kind: r.kind,
      data: r.data,
    }));
    const u = results.users.map((d) => ({
      group: "People",
      kind: "user",
      data: d,
    }));
    const p = results.posts.map((d) => ({
      group: "Posts",
      kind: "post",
      data: d,
    }));
    const t = results.tags.map((d) => ({
      group: "Hashtags",
      kind: "tag",
      data: d,
    }));
                  ...
                  {!busy && results && (
                    <div className="max-h-[70vh] overflow-y-auto">
                      ...
                      {results.top.length > 0 && (
                        <div>
                          <div className="px-4 pt-3 pb-1 text-xs uppercase text-gray-500">
                            Top
                          </div>
                          {results.top.map((r, i) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() =>
                                  handleNavigate(r.kind as any, r.data)
                                }
                                className={`w-full text-left px-4 py-2 hover:bg-gray-50 flex items-center gap-3 ${
                                  activeIndex === flatIdx ? "bg-gray-50" : ""
                                }`}
                              >
                                {r.kind === "user" && (
                                  <>
                                    <Avatar className="h-7 w-7">
                                      <AvatarImage
                                        src={
                                          API_BASE_URL +
                                          "/uploads/" +
                                          stripUploads(r.data.avatar || "")
                                        }
                                      />
                                      <AvatarFallback>
                                        {initials(
                                          r.data.fullName || r.data.username
                                        )}
                                      </AvatarFallback>
                                    </Avatar>
                                    <div>
                                ...
                                {r.kind === "post" && (
                                  <>
                                    <div className="h-7 w-7 rounded bg-blue-100 flex items-center justify-center text-blue-600 text-xs">
                                      Post
                                    </div>
                                    <div
                                      className="text-sm line-clamp-2"
                                      dangerouslySetInnerHTML={{
                                        __html: r.data.snippet,
                                      }}
                                    />
                                  </>
                                )}
                                {r.kind === "tag" && (
                                  <>
                                    <div className="h-7 w-7 rounded bg-purple-100 flex items-center justify-center text-purple-600 text-xs">
                                      #
                                    </div>
                      ...
                      {results.posts.length > 0 && (
                        <div>
                          ...
                          {results.posts.map((p, i) => {
                            const flatIdx =
                              results.top.length + results.users.length + i;
                            return (
                              <button
                                key={`p-${p.id}`}
                                onMouseEnter={() => setActiveIndex(flatIdx)}
                                ...
                                <Avatar className="h-7 w-7 mt-0.5">
                                  <AvatarImage
                                    src={
                                      API_BASE_URL +
                                      "/uploads/" +
                                      stripUploads(p.author?.avatar || "")
                                    }
                                  />
                                  <AvatarFallback>
                                    {initials(p.author?.username || "U")}
                                  </AvatarFallback>
                                </Avatar>
                                <div
                                  className="text-sm line-clamp-2"
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <Link to="/points">
                  <DropdownMenuItem className="cursor-pointer">
                    <Currency className="mr-2 h-4 w-4" /> Points{" "}
