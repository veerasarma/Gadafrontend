...
export default function AdminGroupCategoryForm() {
  const { id } = useParams();
  const isEdit = !!id;
  ...
  const loadOptions = async () => {
    const r = await fetch(`${API_BASE_URL}/api/admin/group-categories/options`, { headers });
  ...
  const load = async () => {
    if (!isEdit) return;
    const r = await fetch(`${API_BASE_URL}/api/admin/group-categories/${id}`, { headers });
  ...
  const save = async () => {
    ...
    const url = isEdit
      ? `${API_BASE_URL}/api/admin/group-categories/${id}`
      : `${API_BASE_URL}/api/admin/group-categories`;
    ...
    toast.success('Saved');
    navigate('/admin/group-categories');
  };
    ...
    <div className="bg-white rounded-xl shadow-sm border p-4">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-lg font-semibold">
          Groups › Categories › {isEdit ? 'Edit Category' : 'Add New Category'}
        </h2>
        <Button variant="outline" onClick={()=>navigate('/admin/group-categories')}>Go Back</Button>
      </div>
