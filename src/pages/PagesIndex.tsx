// src/pages/PagesIndex.tsx
import { useEffect, useRef, useState } from 'react';
import { Navbar } from '@/components/layout/Navbar';
import { useAuth } from '@/contexts/AuthContext';
import { useAuthHeader } from '@/hooks/useAuthHeader';
import { fetchCategories, listPages } from '@/services/pagesService';
  ...
  const loadMore = async (reset=false) => {
    ...
    try {
      const { items: pageItems = [], nextCursor } = await listPages(
        { q: q.trim(), categoryId: categoryId || undefined, sort, cursor: reset ? undefined : cursor, limit: 12 },
        headersRef.current
      );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {Array.isArray(items) && items.map(it => (
            <Link key={it.page_id} to={`/pages/${it.page_name}`} className="bg-white rounded-lg shadow hover:shadow-md transition">
